...
interface KineticHeadlineProps {
  ...
  delay?: number
  stagger?: number
  /** Optional per-line class overrides (e.g. italic accent lines). */
  lineClasses?: string[]
  /**
   * Words rendered as Fraunces italic accents (design.md section 3 — max one per
   * headline). Matched on the bare word (punctuation stripped), word-split
   * mode only.
   */
  accentWords?: string[]
  /** Classes applied to accent words. */
  ...
  return node.split(' ').map((w, i) => {
    ...
    return (
      <span
        key={i}
        className={cn(
          'kinetic-unit inline-block will-change-transform',
          isAccent && (accentClassName ?? 'font-accent font-normal italic'),
        )}
      >
        {w}
        {' '}
...
/**
 * Kinetic display headline (design.md §5): Archivo units rise from y:110%
 * inside overflow-hidden wrappers, 1s power3.out. GSAP-owned; do not nest
 * inside Framer Motion animated elements.
 */
export default function KineticHeadline({
  ...
  return (
    <TagAny ref={ref} className={cn('font-display', className)}>
      {lines.map((line, i) => (
        <span key={i} className={cn('block overflow-hidden pb-[0.08em] -mb-[0.08em]', lineClasses?.[i])}>
          {splitLine(line, split, accentWords, accentClassName)}
        </span>
      ))}
    </TagAny>
  )
